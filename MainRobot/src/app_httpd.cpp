...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (true) {
    ...
    if (!fb) {
      ...
      img.width = fb->width;
      plants_index = planted_get_plants_xy(&img, plants, 10);
      log_printf("PINDEX = %d\n", plants_index);
      fmt2jpg(rgb, ARRAY_LENGTH, img.width, img.height, PIXFORMAT_RGB888, 20,
...
static esp_err_t pll_handler(httpd_req_t *req) {
  ...
  log_i(
      "Set Pll: bypass: %d, mul: %d, sys: %d, root: %d, pre: %d, seld5: %d, "
      "pclken: %d, pclk: %d",
